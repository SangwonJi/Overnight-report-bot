...
def translate_text_with_gemini(text_to_translate):
    ...
    try:
        ...
        return response.text.strip().replace("*", "") # 마크다운이 깨지지 않게 별표 제거
    except Exception:
...
def check_cloudflare_outages(country_code):
    """[수정됨] Cloudflare Radar API의 URL을 단순화하여 요청합니다."""
    try:
        # URL에서 파라미터를 제거하여 가장 기본적인 형태로 요청
...
def check_for_holidays(country_code):
    """'National holiday' 같은 실제 공휴일만 필터링합니다."""
    try:
